...
import { Registration, RegistrationInput } from '../../interfaces';
...
export default function UpdateRegistration() {
  ...
  const [valueOverwrites] = React.useState(() => {
    ...
    const paymentAmount = registration
      ? (registration.total / 100).toFixed(2)
      : 0;
    const phone = registration ? formatPhoneNumber(registration.phone) : '';
    const ecPhone = registration
      ? formatPhoneNumber(registration.emergencyContact.phone)
      : '';
    const crewMembers =
  ...
  const mutation = useMutation(
    async (registration: RegistrationInput) => {
      const response = await fetch(
        `/api/registrations/update?id=${router.query.id}`,
        {
          method: 'POST',
    ...
    },
    {
      onSuccess: data => {
        ...
        queryClient.invalidateQueries(['registration', data._id]);
